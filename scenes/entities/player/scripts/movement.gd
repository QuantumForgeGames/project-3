...
@onready var fuel_value_label = $"../../CanvasLayer/UI/MarginContainer/VBoxContainer/FuelValueLabel"



func _process (_delta_ :float) -> void:
	fuel_value_label.text = str(int(fuel))
	#owner.fuel_label.text = str(fuel)
